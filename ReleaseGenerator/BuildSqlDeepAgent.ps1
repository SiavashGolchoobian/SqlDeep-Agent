...
if (Test-Path $myBasePath\CLI\SqlDeepAgent.ps1 -PathType Leaf) {Rename-Item -Path $myBasePath\CLI\SqlDeepAgent.ps1 -NewName SqlDeepAgent.psm1;Write-Host 'rename cli ps1 to psm1'}
...
if (Test-Path $myBasePath\CLI\SqlDeepAgent.psm1 -PathType Leaf) {Rename-Item -Path $myBasePath\CLI\SqlDeepAgent.psm1 -NewName SqlDeepAgent.ps1;Write-Host 'rename cli psm1 to ps1'}
Set-AuthenticodeSignature -FilePath  $myBasePath\CLI\SqlDeepAgent.ps1 -Certificate $myCertificate -IncludeChain All -TimeStampServer http://timestamp.digicert.com
